...
-- Autosnippets
{
  ...
  s("dm", fmt("$ {} $", { i(1) }), { condition = line_begin }),
  s({ trig = "bb", wordTrig = false }, fmt("({})", { i(1) }), { condition = is_math }),
  ...
  s({
    ...
    p(function(_, snip) return string.format("%s_%s", snip.captures[1], snip.captures[2]) end, {}),
    i(0),
  }, { condition = is_math }),
